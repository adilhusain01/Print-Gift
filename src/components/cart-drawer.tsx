...
export function CartDrawer({ storeMode = "retail" }: { storeMode?: StoreMode }) {
  ...
  return (
    <Sheet>
      ...
      <SheetContent className="data-[side=right]:w-full border-l border-border bg-background sm:max-w-md">
        <SheetHeader>
          <SheetTitle className="font-heading text-3xl font-medium">{bulkMode ? "Your inquiry bag" : "Your gift bag"}</SheetTitle>
          <SheetDescription>{bulkMode ? "Review products before sending a bulk inquiry." : "Review your picks before checkout."}</SheetDescription>
        </SheetHeader>
