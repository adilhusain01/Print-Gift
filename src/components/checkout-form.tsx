...
export function CheckoutForm({ settings }: { settings: StoreSettings }) {
  ...
  if (reference) return <div className="surface mx-auto max-w-2xl p-8 text-center sm:p-12"><div className="mx-auto grid size-14 place-items-center rounded-full bg-[#59614a] text-white"><MessageCircle className="size-6" /></div><h1 className="mt-6 font-heading text-4xl">{bulkMode ? "Your inquiry is ready" : "Your order is ready"}</h1><p className="mt-3 leading-7 text-muted-foreground">Your reference is <strong className="text-foreground">{reference}</strong>. WhatsApp should now be open so we can confirm {bulkMode ? "bulk pricing, availability, customization, and timeline" : "availability, personalization, and payment"}.</p><Button nativeButton={false} render={<Link href="/shop" />} className="brutal-button mt-7 rounded-full bg-primary px-6 text-primary-foreground">Continue browsing</Button></div>;
  if (!items.length) return <div className="surface mx-auto max-w-xl p-10 text-center"><ShoppingBag className="mx-auto size-9 text-muted-foreground" /><h1 className="mt-5 font-heading text-4xl">{bulkMode ? "Your inquiry bag is empty" : "Your gift bag is empty"}</h1><Button nativeButton={false} render={<Link href="/shop" />} className="brutal-button mt-7 rounded-full bg-primary px-6 text-primary-foreground">Explore gifts <ArrowRight /></Button></div>;
