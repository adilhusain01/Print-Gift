  ...
  return (
    <>
      <div className="mb-10 grid gap-5 border-y border-border py-5 lg:grid-cols-[minmax(260px,1fr)_auto] lg:items-center">
        ...
        <div className="flex flex-wrap gap-2">{filters.map((filter) => <Button key={filter} onClick={() => setCategory(filter)} variant="ghost" className={`min-h-11 rounded-full px-4 text-xs capitalize ${category === filter ? "bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground" : "border border-border/70 text-muted-foreground hover:bg-muted"}`}>{filter.replace("-", " ")}</Button>)}</div>
      </div>
